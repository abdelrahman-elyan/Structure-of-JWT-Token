...
const cookieParser = require('cookie-parser'); // Import cookie-parser for handling Refresh Token
...
// FIXED: Replaced 'WEAK_SECRET' with strong, separate secrets
const ACCESS_SECRET = 'SECRET_DIFFERENT_FROM_ACCESS'; //
const REFRESH_SECRET = 'REFRESH_SECRET_DIFFERENT_FROM_ACCESS'; //

// Helpers
// Helper: issue access token (short-lived: 15m)
function issueAccessToken(username, role) {
  return jwt.sign({ sub: username, role }, ACCESS_SECRET, {
    algorithm: 'HS256',
    expiresIn: '15m', // FIXED: Was 7d (too long)
    issuer: 'jwt-lab' // ADDED: for strict validation
  });
}
// Helper: issue refresh token (long-lived: 7d)
...
app.post('/login', (req, res) => {
  ...
  DB.get("SELECT * FROM users WHERE username = ?", [username], (err, row) => {
    ...
    const accessToken = issueAccessToken(row.username, row.role); // Short-lived token
    const tokenId = Math.random().toString(36).slice(2);
    const refreshToken = issueRefreshToken(row.username, tokenId); // Long-lived token
    // store tokenId associated with username
    refreshStore.set(tokenId, { username: row.username, created: Date.now() });

    // CRITICAL: Store sensitive refresh token in HttpOnly cookie
    // FIXED: vuln server returned this in JSON (vulnerable to XSS)
    res.cookie('refreshToken', refreshToken, {
      httpOnly: true,      // httpOnly: true prevents XSS from stealing it
...
// SECURE: Middleware to authenticate access token (strict verification)
function authMiddleware(req, res, next) {
  const auth = (req.headers.authorization || '');
  const token = auth.replace('Bearer ', '');
  if (!token) return res.status(401).json({ error: 'No token' });
  try {
    // THE FIX for alg:none vulnerability:
    // 1. Use 'jwt.verify' (checks signature), not 'jwt.decode' (only parses)
    // 2. Force a specific algorithm; DO NOT trust the token's header
    const payload = jwt.verify(token, ACCESS_SECRET, { algorithms: ['HS256'], issuer: 'jwt-lab' }); //
    req.user = payload;
    next();
  } catch (e) {
    // Any forged token (like alg:none) will fail here
    return res.status(401).json({ error: 'Invalid token' });
  }
}

// Protected admin
// Use the secure authMiddleware
...
app.post('/refresh', (req, res) => {
  const token = req.cookies.refreshToken; // Read token from the secure HttpOnly cookie
